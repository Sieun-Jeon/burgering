...
class Preprocessing(Patty):
    ...
    def manage(self,data,list):
        rslt = Burger.alert(self.burger, self, "이상값이 총 " + len(list) + "개 입니다.", ["전부 삭제", "개별 확인", "변경하지 않음"])
        if rslt == "전부 삭제":
            result_list = ['행 삭제', '열 삭제', '변경하지 않음']
            mssg = "전체 outlier에 대해 할 행동을 골라주세요"
            result = Burger.alert(self.burger, self, mssg, result_list)
            if result == '행 삭제':
                col =[]
                for each in list:
                ...
                Burger.alert(self.burger, self, mssg, [])
            elif result =="열 삭제":
                row = []
                for each in list:
            ...
            for each in list:
                ...
                if result == '행 삭제':
                    ...
                    rslt = Burger.alert(self.burger, self, "수정할 값을 입력하세요")

                    Burger.alert(self.burger, self, "변경할 값을 입력해주세요 : ", )
